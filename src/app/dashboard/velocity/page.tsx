  ...
  if (!data?.boards?.length) {
    return (
      <div className="smg-card p-6">
        <h2 className="text-lg font-semibold text-smg-gray-700">No velocity data</h2>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-smg-gray-900">Sprint Velocity</h1>
          <p className="mt-1 text-sm text-smg-gray-500">
              ...
              {[...sprints].reverse().map((s) => {
                ...
                return (
                  <tr key={`${s.sprintId}-${s.boardName}`} className="border-t border-smg-gray-100 hover:bg-smg-gray-50/50">
                    ...
                    <td className="px-4 py-3 text-right">
                      {s.scopeAdded > 0 ? <span className="font-mono text-smg-warning">+{s.scopeAdded}</span> : <span className="text-smg-gray-300">0</span>}
                    </td>
                    <td className="px-4 py-3 text-smg-gray-500">{s.sprintEndDate ? new Date(s.sprintEndDate).toLocaleDateString() : "\u2014"}</td>
                  </tr>
                );
              })}
            </tbody>
